...
│       │   └── auth_utils.py                 # Bcrypt hashing, JWT encode/decode & token validation
│       ├── db/
...
│           └── endpoints/
│               ├── auth.py                   # Admin JWT login, session verification & admin CRUD
...
└── frontend/                                 # React 18 + Vite Frontend Application
    ...
    └── src/
        ...
        ├── context/
        │   └── AuthContext.jsx               # Global admin auth state, token lifecycle & logout
        ├── components/
        ...
        └── pages/
            ├── Login.jsx                     # Full-page admin authentication portal
...
3. **Multi-Cycle Arrears & Dynamic Overdue Engine:**
   - The `invoices` database table includes an `arrears` column (`NUMERIC`, default `0.00`).
   - When generating new vouchers for a billing cycle, the system inspects prior unpaid/unverified invoices for each resident unit and carries forward the unpaid balance into `arrears`.
   - Dynamic formula: `total_payable = society_maintenance_fee + arrears`.
   - On WhatsApp vouchers: if prior unpaid dues exist, the resident receives a transparent breakdown with `• Current Month Fee: Rs. X`, `• Unpaid Arrears: Rs. Y`, and `*Total Payable: Rs. Z*`.
   - Overdue status is computed dynamically based on the payment deadline (`due_date < today`) or any outstanding carried-forward arrears.
   - **FIFO Debt Settlement**: When a resident pays an arrears voucher, earlier unsettled cycles are cleared chronologically to keep the resident's accounting ledger accurate.
4. **Resident-Centric Finance Roster (`residentRoster` in `Invoices.jsx`):**
   - The web portal groups raw database invoices by unique resident apartment unit (`building + unit_number` / `resident_id`).
   - **Zero Duplicate Rows:** Eliminates duplicate rows for residents who have multiple historical invoices, presenting a clean 1-row-per-resident roster.
   - Summarizes current base fee, accumulated arrears, total balance due, latest cycle due date, and delivery status.
   - Financial KPIs (Realization Rate, Realized Collections, Total Overdue) calculate across distinct resident units rather than skewed raw invoice counts.
5. **Interactive Payment & Voucher History Ledger (`GET /api/v1/invoices/resident/{resident_id}/history`):**
   - Society admins can click anywhere on a resident's row or on the dedicated **"Ledger"** button to open the resident's comprehensive payment history modal.
   - **KPI Overview**: Displays 3 summary cards for Total Billed, Total Paid, and Current Balance Due across all cycles.
   - **Quick Filtering**: Filter cycle rows by `All`, `Paid`, `Unpaid / Overdue`, and `With Receipts`.
   - **Proof Verification**:
     - *Online / WhatsApp Transfers:* Displays thumbnail and "View Screenshot" button opening a full-size modal lightbox, along with detected TxID, beneficiary account, and verification timestamp.
     - *Cash Payments:* Displays `💵 Cash Collected` with collector name and timestamp from the audit trail.
   - **Direct Ledger Actions:** Admins can mark specific historical cycles paid or review pending slips directly from inside the history modal.
6. **Idempotency & Delivery Tracking:**
...
7. **Payment Accountability & Audit Trail:**
...
8. **Dashboard-Matching Shimmer Skeleton:**
...
### H. Community Polls WhatsApp Broadcast & Voting Engine (`backend/app/api/v1/endpoints/polls.py`, `gemini.py` & `Polls.jsx`)
1. **Interactive WhatsApp Poll Broadcast (`POST /api/v1/polls/` & `POST /api/v1/polls/{id}/broadcast`):**
   - Polls can be broadcast automatically upon creation via a toggle checkbox or on-demand at any time using the dedicated **"Broadcast Poll"** button on active poll cards.
   - Dispatches formatted WhatsApp messages with emojis, numbered options (1️⃣, 2️⃣, 3️⃣...), voting deadline, and simple conversational reply instructions.
   - Strictly enforces WhatsApp single asterisk `*bold*` and single underscore `_italic_` markdown formatting.
2. **Granular Audience Targeting & 1-Message-per-Apartment Deduplication:**
   - Supports whole-society broadcasts or block-specific scoping (e.g. `Block A`, `Block B`, `Block C`).
   - Strictly enforces 1-message-per-apartment deduplication (keyed by `(building, unit_number)`), prioritizing the registered property owner or active tenant as primary contact to prevent household alert spam.
3. **Conversational Voting via Gemini AI Concierge Engine (`cast_poll_vote` action):**
   - Inbound resident messages replying with either option numbers (e.g. `1`, `2`) or option text (e.g. `Upgrade completely`) are matched by the Gemini concierge engine.
   - Enforces strict 1-vote-per-unit via the `poll_votes` unique constraint.
   - If already voted, Gemini informs the resident of their existing vote and explains the 1-vote-per-unit policy.
   - If successfully cast, Gemini confirms with a bilingual confirmation notice (`✅ Shukriya! Aapka vote *...* ke liye kamyabi se darj kar liya gaya hai.`).
4. **Live Dashboard & Audit Export (`Polls.jsx`):**
   - Renders real-time percentage progress bars and total votes tally per option.
   - Includes live interactive WhatsApp Mobile Chat Preview bubble (`[Hide Preview / Show Preview]`) in the creation modal.
   - Enables closing polls and auto-exporting PDF/Excel voting audit reports upon expiry.

### I. Resident Profile Management & Document Repository (`backend/app/api/v1/endpoints/residents.py` & `frontend/src/pages/Residents.jsx`)
1. **Mandatory CNIC Verification:**
   - CNIC is strictly compulsory for resident registration and editing (validated against standard Pakistani 5-7-1 format: `XXXXX-XXXXXXX-X`).
2. **Dual Slash & Empty-Path Route Resiliency:**
   - The resident creation endpoint registers both `@router.post("")` and `@router.post("/")` to eliminate 307 Temporary Redirects and 405 Method Not Allowed errors across cross-origin deployments and Vercel proxy rewrites.
3. **Pre-flight Phone Uniqueness Check & Clean Error Reporting:**
   - Before inserting into Supabase, the backend verifies whether the normalized phone number is already registered to an existing resident, returning an informative 400 error identifying the household (`Phone number +92... is already registered to [Name] (Block, Unit)`).
   - The Add Resident modal in React (`Residents.jsx`) features a top error banner and button loading states (`isSavingResident`), keeping the modal open on validation errors so administrators can correct inputs without data loss.
4. **Optional Document Repository (PDF / Image Upload):**
   - Admins can upload verification documents such as tenancy contracts, CNIC copies, or allotment letters (`.pdf`, `.png`, `.jpg`).
   - Stored in Supabase Storage (`society-receipts/resident-documents/`) and stored in `residents.document_url`.
   - Uploading is non-mandatory and flexible: documents can be uploaded at resident creation or attached/modified at any later time via the Edit modal.
5. **Integrated Edit & Safe Delete Workflow:**
   - Admins can edit resident name, phone number, CNIC, ownership type (Owner vs Tenant), building/unit, and documents directly from the resident roster.
   - **Protected Delete Action:** To preserve data integrity and prevent accidental data loss, the Delete Resident button is situated inside the Edit modal rather than exposed as an open table row action. Deletion requires an explicit confirmation modal.
6. **Retirement of "Block Access":**
   - The legacy "Block Access" action has been permanently removed from the UI. Gated residential societies do not bar verified occupants from essential premises access, WhatsApp concierge services, emergency help, or community dues inquiries.

### J. Admin Authentication, Portal Polish & Meta Diagnostics
1. **Super Admin Quick-Access Card (`frontend/src/pages/Login.jsx`):**
   - For developer testing and admin onboarding, the login screen displays a dedicated credentials helper card with a 1-click **"Auto Fill Credentials"** button populating `admin@hamsayaa.com` / `admin123`.
2. **Vehicles & Gate Logs Polish (`frontend/src/pages/Vehicles.jsx`):**
   - Removed the obsolete ANPR camera simulation and hardware placeholder banner. The interface focuses cleanly on guard manual gate logging and Excel imports.
3. **Meta WhatsApp Graph API Error Diagnostics (`backend/app/api/v1/endpoints/whatsapp.py`):**
   - Webhook status updates capture and log specific Meta Cloud API delivery error codes (e.g. `Code 131047: Re-engagement message failed because more than 24 hours have passed since customer last replied`), aiding fast diagnosis of messaging failures.
...
### 4.1 Relational Database Schemas (14 Tables)
...
- `cnic` (`TEXT`, Not Null in UI) — Resident National Identity Card number (e.g. `42101-1234567-1`), mandatory with 5-7-1 format validation
- `document_url` (`TEXT`, Nullable) — Verification documents (PDF tenancy agreement, CNIC scan) stored in Supabase Storage; optional, uploadable during creation or editing
- `is_owner` (`BOOLEAN`, Default: `true`) — Resident owns the property
- `is_tenant` (`BOOLEAN`, Default: `false`) — Resident is renting
- `is_blocked` (`BOOLEAN`, Default: `false`) — *(Retired)* Residents are never blocked from WhatsApp concierge, emergency access, dues inquiries, or voting.
- `created_at` (`TIMESTAMPTZ`, Default: `now()`)
...
- `arrears` (`NUMERIC`, Default: `0.00`) — Outstanding unpaid balance carried forward from previous billing cycles (multi-cycle accounting)
...
#### 14. `admins`
Administrative and committee user credentials for web dashboard access.
- `id` (`UUID`, Primary Key, default: `gen_random_uuid()`)
- `society_id` (`UUID`, Foreign Key -> `societies.id`, Nullable for global superadmins)
- `name` (`TEXT`, Not Null) — Full name of administrator or committee member
- `email` (`TEXT`, Unique, Not Null) — Login username / official email
- `password_hash` (`TEXT`, Not Null) — Bcrypt salted password hash (never stored in plain text)
- `phone` (`TEXT`, Nullable) — Contact mobile phone
- `role` (`TEXT`, Default: `admin`) — Access tier: `super_admin`, `admin`, `accountant`, `guard`
- `is_active` (`BOOLEAN`, Default: `true`) — Account status; deactivated accounts receive 403 Forbidden
- `created_at` (`TIMESTAMPTZ`, Default: `now()`)
- `last_login_at` (`TIMESTAMPTZ`, Nullable) — Stamped on successful JWT token issuance
*Indexes:* `idx_admins_email` on `email`, `idx_admins_society_id` on `society_id`.
...
### 6.1 Authentication & Admin Access (`/api/v1/auth`)
- `POST /login`: Authenticates an admin with email and password. Returns a 24-hour signed JWT Bearer token and admin profile (`id`, `name`, `email`, `role`, `society_id`).
- `GET /me`: Validates incoming `Authorization: Bearer <token>` and returns the active admin session.
- `GET /admins`: Lists all administrators for a society (requires Bearer token authentication).
- `POST /admins`: Registers a new admin user with bcrypt password hashing and unique email enforcement (requires Bearer token).
- `PATCH /admins/{admin_id}`: Updates admin details (`name`, `email`, `phone`, `role`, `is_active`, optional password reset).
- `DELETE /admins/{admin_id}`: Deletes an admin user (with built-in self-deletion guard).

### 6.2 WhatsApp Webhooks & Testing (`/api/v1/whatsapp`)
...
### 6.3 Residents Management (`/api/v1/residents`)
...
### 6.4 Finance & Invoices (`/api/v1/invoices`)
...
### 6.5 Complaints & Tickets (`/api/v1/complaints`)
...
### 6.6 Vehicles & Access Control (`/api/v1/vehicles`)
...
### 6.8 Employees, Assets, Amenities & Settings
...
### 7.5 Admin Authentication, Session Lifecycle & Route Guarding (`frontend/src/context/AuthContext.jsx` & `Login.jsx`)
- **Global Auth Provider (`<AuthProvider>`):** Manages reactive admin session state (`admin`, `token`, `loading`). Persists the signed JWT in browser `localStorage` (`hamsayaa_token`).
- **Route Guarding (`<ProtectedRoute>`):** Intercepts all application routes in `App.jsx`. Unauthenticated requests or expired sessions are cleanly redirected to `/login`. The full-page `<Login />` portal renders isolated without the navigation header.
- **Top Navigation Integration (`Navbar.jsx`):** Dynamically displays the authenticated administrator's initials avatar badge, real name, and role tier, replacing all hardcoded placeholder data. Features a dedicated one-click logout action that flushes session credentials and routes to login.

### 7.6 Settings Admin Users Management (`frontend/src/pages/Settings.jsx`)
- **Dedicated Management Tab:** Fourth tab ("Admin Users") integrated into society settings interface (alongside Society Profile, Billing & Banking, and Gate & Visitor Rules).
- **Administrative Roster Table:** Displays real-time administrator listings including full name, verified email, contact phone, role tier pill (`Super Admin`, `Admin`, `Accountant`, `Guard`), and active status badge.
- **Add / Edit Administrator Modal:** Implements the zero-cut-off standard (`max-h-[90vh] flex flex-col my-auto` with pinned header/footer and independent form scrolling). Allows committee leadership to provision new admins with mandatory password hashing, update roles, toggle active status, and perform administrative password resets.
- **Self-Deletion Guard:** Backend and frontend forbid administrators from deleting their own active profile to prevent administrative lockout.
