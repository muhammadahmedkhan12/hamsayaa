...
async def process_inbound_message(
    ...
    try:
        ...
        # If sender is not a registered resident, do not reply and ignore message completely
        if not resident:
            logger.info(f"--> [Non-Resident Ignored] No registered resident found for {formatted_phone}. Dropping message without reply.")
            return

        logger.info(f"--> RESIDENT MATCHED: {resident.get('name')} (Unit {resident.get('unit_number')})")
        ...
        if not messages:
            ...
            if statuses:
                for s in statuses:
                    ...
                    s_recipient = s.get("recipient_id", "")
                    s_errors = s.get("errors", [])
                    print(f"--> WEBHOOK MESSAGE STATUS UPDATE: {s_status} for recipient {s_recipient}")
                    if s_errors:
                        for err in s_errors:
                            print(f"    ❌ META DELIVERY ERROR: Code {err.get('code')} - {err.get('title')}: {err.get('message')}")
