      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
            <div className="w-8 h-8 rounded-full bg-navy text-white flex items-center justify-center font-bold text-xs shadow-sm uppercase">
              {admin?.name?.split(' ').map(n => n[0]).join('').slice(0, 2) || 'AD'}
            </div>
            <div className="hidden lg:block leading-none">
              <p className="text-xs font-semibold text-navy">{admin?.name || 'Admin'}</p>
              <p className="text-[9px] text-slate-500 mt-0.5">{admin?.role || 'admin'}</p>
            </div>
            <button 
