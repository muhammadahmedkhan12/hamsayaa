...
export default function Dashboard() {
  ...
  const openTickets = summary?.open_tickets_count ?? 5;
  const humanReview = summary?.needs_human_review_count ?? 1;
  const overdueTotal = summary?.overdue_dues_total ?? 145000;
  const overdueCount = summary?.overdue_count ?? 8;
  const activePasses = summary?.active_passes_count ?? 8;
  const flaggedOverstays = summary?.flagged_overstays_count ?? 2;
